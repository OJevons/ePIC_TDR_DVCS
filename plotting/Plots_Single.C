...
// MAIN
// 
// ePIC TDR plots - single particle
//---------------------------------------------------------------------
void Plots_Single(TString campaign = "26.07.1", TString energy = "9x130", TString hel = "emhTm"){
  // Print plot settings
  cout<<"---------------------------------"<<endl;
  ...
  // 1) Scattered electron - eta, E/p, 2D E/eta
  ...
  // 2) Real photon - eta, 2D E/eta, theta resolution
  ...
  // CANVAS: FULL EVENT ETA
  TCanvas* cEta = new TCanvas("cEta","",1500,500);
  cEta->Divide(3,1,-1);
  // Scale distributions to EIC lumi
  ...
  Float_t maxEta{0};
  for(TH1D* h : {h_eta_MCe,h_eta_MCg,h_eta_MCp,h_eta_RPg,h_eta_RPe,h_eta_RPp,h_eta_RPPp}){
    if(h->GetMaximum() > maxEta) maxEta = h->GetMaximum();
  }
  ...
  // Save figure
  if(kSAVE) cEta->SaveAs(sFigsPath + "TDR_" + energy + "_Eta.png");
  cEta->Close();
  ...
  tePICLabel_EvEtaelec->SetNDC();
  tePICLabel_EvEtaelec->SetTextSize(0.04);
  tePICLabel_EvEtaelec->Draw("same");
  // Save figure
  if(kSAVE) cEvEta_e->SaveAs(sFigsPath + "TDR_" + energy + "_EvEta_e.png");
  cEvEta_e->Close();
  ...
  tePICLabel_EvEtaphot->SetNDC();
  tePICLabel_EvEtaphot->SetTextSize(0.04);
  tePICLabel_EvEtaphot->Draw("same");
  // Save figure
  if(kSAVE) cEvEta_g->SaveAs(sFigsPath + "TDR_" + energy + "_EvEta_g.png");
  cEvEta_g->Close();
  ...
  tePICLabel_EvEtaprot->SetNDC();
  tePICLabel_EvEtaprot->SetTextSize(0.04);
  tePICLabel_EvEtaprot->Draw("same");
  // Save figure
  if(kSAVE) cEvEta_p->SaveAs(sFigsPath + "TDR_" + energy + "_EvEta_p.png");
  cEvEta_p->Close();
  ...
  lEOverP->Draw();
  // Save figure
  if(kSAVE) cEOverP->SaveAs(sFigsPath + "TDR_" + energy +"_EoverP.png");
  cEOverP->Close();

  
  // CANVAS: PHOTON ANGULAR RESOLUTION
  ...
  // Save figure
  if(kSAVE) cPhotRes->SaveAs(sFigsPath + "TDR_" + energy +"_PhotRes.png");
  cPhotRes->Close();
  ...
  lProtTheta->Draw();
  // Save figure
  if(kSAVE) cProtTheta->SaveAs(sFigsPath + "TDR_" + energy +"_ProtTheta.png");
  cProtTheta->Close();
